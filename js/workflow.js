...
const PMSWorkflow = (() => {
  const STEPS = [
    ...
    { id: 5, name: 'Commander Verification', status: 'Pending Commander Review', role: 'Jail Commander', action: 'Verify assessment information and institutional readiness' },
    { id: 6, name: 'Form 3 — Institutional Report', status: 'Draft', role: 'Jail Commander', action: 'Complete Form 3 (Institutional Report)' },
    ...
    { id: 14, name: 'Approval Workflow', status: 'Pending Approval', role: 'Board / Commander', action: 'Complete required approvals before release' },
    { id: 15, name: 'Release Authorization', status: 'Approved', role: 'Jail Commander', action: 'Authorize release from institution' },
  ...
  const TRANSITION_ROLES = {
    ...
    'Submitted→Returned for Correction': ['DJAG Parole Clerk', 'Jail Commander', 'System Administrator'],
    'Submitted→Pending Commander Review': ['PNGCS Parole Clerk', 'CS Parole Officer', 'System Administrator'],
    'Pending Commander Review→Pre-Parole Report Prepared': ['Jail Commander', 'System Administrator'],
    'Pending Commander Review→Returned for Correction': ['Jail Commander', 'System Administrator'],
    'Pending Commander Review→Refused': ['Jail Commander', 'System Administrator'],
    ...
    'Parole Granted→Approved': ['Parole Board Member', 'Jail Commander', 'System Administrator'],
    'Parole Refused→Refused': ['Parole Board Member', 'System Administrator'],
    'Pending Approval→Approved': ['Parole Board Member', 'Jail Commander', 'System Administrator'],
    'Pending Approval→Returned for Correction': ['Parole Board Member', 'Jail Commander', 'System Administrator'],
    'Pending Approval→Refused': ['Parole Board Member', 'System Administrator'],
    'Approved→Released': ['Jail Commander', 'System Administrator'],
  };
  ...
  function getAdvanceBlockers(app, toStatus) {
    ...
    if (toStatus === 'Submitted') {
      ...
      need('commanderVerified', 'Jail Commander must verify Form 1.');
    }
    ...
    if (toStatus === 'Hearing Scheduled') { need('commanderVerified', 'Jail Commander verification is required.'); need('form3', 'Form 3 must be completed.'); }
